...
{/*
   ...
   */                         
   ...
   //        __emulation_paused = 0;
    ...
    {
	   [self endMenu];
	   /* Les Bird - no support for WiiMotes on non-jailbroken devices
...
-(void)iCadeConnect
{
	...
	SBJoystick *joystick = [SBJoystick sharedInstance];
...
-(void)pollSBJoystick
{
	SBJoystick *joystick = [SBJoystick sharedInstance];
	gp2x_pad_status = 0;
	if (joystick.buttonSelectState)
	{
		gp2x_pad_status |= GP2X_SELECT;
	}
	if (joystick.buttonStartState)
	{
		gp2x_pad_status |= GP2X_START;
	}
	if (joystick.buttonL1State)
	{
		gp2x_pad_status |= GP2X_L;
	}
	if (joystick.buttonR1State)
	{
		gp2x_pad_status |= GP2X_R;
	}
	if (joystick.buttonUState)
	{
		gp2x_pad_status |= GP2X_UP;
	}
	if (joystick.buttonLState)
	{
		gp2x_pad_status |= GP2X_LEFT;
	}
	if (joystick.buttonDState)
	{
		gp2x_pad_status |= GP2X_DOWN;
	}
	if (joystick.buttonRState)
	{
		gp2x_pad_status |= GP2X_RIGHT;
	}
	if (joystick.button2State)
	{
		gp2x_pad_status |= GP2X_B;
	}
	if (joystick.button3State)
	{
		gp2x_pad_status |= GP2X_X;
	}
	if (joystick.button4State)
	{
		gp2x_pad_status |= GP2X_A;
	}
	if (joystick.button1State)
	{
		gp2x_pad_status |= GP2X_Y;
	}
	float dead_zone_value = (float)(4.0f + (iOS_analogDeadZoneValue * 4.0f))/32.0f;
	if (joystick.leftJoystickVector.x < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_LEFT;
	}
	else if (joystick.leftJoystickVector.x > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_RIGHT;
	}
	if (joystick.leftJoystickVector.y < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_UP;
	}
	else if (joystick.leftJoystickVector.y > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_DOWN;
	}
	// right nub movements
	if (joystick.rightJoystickVector.x < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_A;
	}
	else if (joystick.rightJoystickVector.x > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_B;
	}
	if (joystick.rightJoystickVector.y < -dead_zone_value)
	{
		gp2x_pad_status |= GP2X_Y;
	}
	else if (joystick.rightJoystickVector.y > dead_zone_value)
	{
		gp2x_pad_status |= GP2X_X;
	}
	joy_analog_x[0] = joystick.leftJoystickVector.x;
	joy_analog_y[0] = joystick.leftJoystickVector.y;
	joy_analog_x[1] = joystick.rightJoystickVector.x;
	joy_analog_y[1] = joystick.rightJoystickVector.y;
}
