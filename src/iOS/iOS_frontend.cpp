...
//	sprintf(name,get_documents_path("skins/iOSsplash.bmp"));
	sprintf(name,"%s/skins/iOSsplash.bmp", get_iOS_BundlePath()); // Les Bird - grab skins from resources folder
	f=fopen(name,"rb");
	if (f) {
...
//	sprintf(name,get_documents_path("skins/iOSmenu.bmp"));
	sprintf(name,"%s/skins/iOSmenu.bmp", get_iOS_BundlePath());
	f=fopen(name,"rb");
	if (f) {
...
static void game_list_view(int *pos) {
    ...
    if(global_manufacturer || global_category || global_year) {
        ...
        gp2x_gamelist_text_out( (8*6)-8, (29*8)-6,"iMAME4all v1.10.0f by D.Valdeita");
    }
}
